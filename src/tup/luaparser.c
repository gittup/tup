...
static int output_nl_add_path(struct tupfile *tf, struct path_list *pl, struct name_list *nl)
{
	...
	if(name_cmp(pl->pel->path, "Tupfile.lua") == 0 ||
	   name_cmp(pl->pel->path, "Tuprules.lua") == 0 ||
	   name_cmp(pl->pel->path, "Tupfile") == 0 ||
		   ...
		   struct name_list *onl)
{
	...
	if(tup_db_write_outputs(tf->f, cmdid, &root, group, &old_group, tf->refactoring) < 0)
		return -1;
	...
	if(tup_db_write_inputs(tf->f, cmdid, &root, &tf->env_root, &tf->g->gen_delete_root, group, old_group, tf->refactoring) < 0)
		return -1;
	free_tupid_tree(&root);
	return 0;
}
