...
int parse(struct node *n, struct graph *g)
{
	...
	if(n->parsing) {
		fprintf(thread_stderr, "Error: Circular dependency found among Tupfiles (last dir ID %lli  = '%s').\nThis is madness!\n", n->tnode.tupid, n->tent->name.s);
		return -1;
	}
	n->parsing = 1;
	...
	if(tf.dfd < 0) {
		fprintf(thread_stderr, "Error: Unable to open directory ID %lli\n", tf.tupid);
		goto out_close_vdb;
	}
			 ...
			 const char *cwd, int clen)
{
		...
		if(strcmp(line, "else") == 0) {
			...
			tupid_tree_for_each(symid, rbn, &symtree) {
				...
				if(tent->type == TUP_NODE_GENERATED) {
					fprintf(thread_stderr, "Error: Attempted to include a Tupfile from a path that contains a symlink generated by tup. Directory %lli, symlink used was %lli, line number %i\n", tf->tupid, tent->tnode.tupid, lno);
					sym_bork = 1;
				}
			}
			if(sym_bork)
			...
			*eq = 0;
			...
			if(strncmp(var, "CONFIG_", 7) == 0) {
				fprintf(thread_stderr, "Error: Unable to override setting of variable '%s' because it begins with 'CONFIG_'. These variables can only be set in the tup.config file.\n", var);
				return -1;
			}
			...
			if(rc < 0) {
				fprintf(thread_stderr, "Error setting variable '%s'\n", var);
				return -1;
			}
			free(var);
			free(value);
	...
	if(if_check(&ifs) < 0) {
		fprintf(thread_stderr, "Error parsing Tupfile [%lli]: missing endif before EOF.\n", tf->tupid);
		tup_db_print(thread_stderr, tf->tupid);
		return -1;
	}
...
syntax_error:
	fprintf(thread_stderr, "Error parsing Tupfile [%lli] line %i\n  Line was: '%s'\n", tf->tupid, lno, line);
	tup_db_print(thread_stderr, tf->tupid);
	return -1;
}
	...
	list_for_each_entry_safe(nle, tmpnle, &nl->entries, list) {
		...
		if(nle->tent->type != TUP_NODE_FILE) {
			if(nle->tent->type == TUP_NODE_GENERATED) {
				fprintf(thread_stderr, "Error: Unable to include generated file '%s'. Your build configuration must be comprised of files you wrote yourself.\n", nle->path);
				return -1;
			} else {
				fprintf(thread_stderr, "tup error: Attempt to include node (ID %lli, name='%s') of type %i?\n", nle->tent->tnode.tupid, nle->path, nle->tent->type);
				return -1;
			}
		}
		...
		if(fd < 0) {
			fprintf(thread_stderr, "Error including '%s': %s\n", nle->path, strerror(errno));
			return -1;
		}
		rc = fslurp(fd, &incb);
		close(fd);
		if(rc < 0) {
			fprintf(thread_stderr, "Error slurping file.\n");
			return -1;
		}
		...
		if(newcwd)
			free(newcwd);
		if(rc < 0) {
			fprintf(thread_stderr, "Error parsing included file '%s'\n", nle->path);
			return -1;
		}
...
static int parse_bang_definition(struct tupfile *tf, char *p, int lno)
{
	...
	value = split_eq(p);
	if(!value) {
		fprintf(thread_stderr, "Parse error line %i: Expecting '=' to set the bang rule.\n", lno);
		return -1;
	}
	...
	if(value[0] == '!') {
		...
		if(!st) {
			fprintf(thread_stderr, "Error: Unable to find !-macro '%s'\n", value);
			return -1;
		}
		...
		if(string_tree_add(&tf->bang_tree, &br->st, p) < 0) {
			fprintf(thread_stderr, "Error inserting bang rule into tree\n");
			goto err_cleanup_br;
		}
		return 0;
	}
	...
	if(split_input_pattern(alloc_value, &input, &command, &command_len, &output,
		...
		return -1;
	if(bin != NULL) {
		fprintf(thread_stderr, "Error: bins aren't allowed in !-macros. Rule was: %s = %s\n", p, alloc_value);
		return -1;
	}
	if(swapio) {
		fprintf(thread_stderr, "Error: !-macro must use '|>' separators\n");
		return -1;
	}
...
static int parse_chain_definition(struct tupfile *tf, char *p, int lno)
{
	...
	value = split_eq(p);
	if(!value) {
		fprintf(thread_stderr, "Parse error line %i: Expecting '=' to set the *-chain.\n", lno);
		return -1;
	}
	...
	if(lbracket) {
		...
		*lbracket = 0;
		...
		if(!rbracket) {
			fprintf(thread_stderr, "Parse error line %i: Expecting ']' character in *-chain definition\n", lno);
			return -1;
		}
		*rbracket = 0;
	}
	...
	if(st) {
		...
		if(string_tree_add(&tf->chain_tree, &ch->st, p) < 0) {
			fprintf(thread_stderr, "Error inserting *-chain into tree\n");
			return -1;
		}
	}
	...
	do {
		...
		if(value[0] != '!') {
			fprintf(thread_stderr, "Error: *-chain must be composed of !-macros, not '%s'\n", value);
			return -1;
		}
			   ...
			   const char *cwd, int clen)
{
	...
	if(!st) {
		...
		if(!st) {
			fprintf(thread_stderr, "Error finding bang variable: '%s'\n",
				r->command);
			return -1;
		}
	}
			       ...
			       int *swapio)
{
	...
	if(brace) {
		...
		brace = strchr(bin, '}');
		if(!brace) {
			fprintf(thread_stderr, "Missing '}' to finish bin\n");
			return -1;
		}
		if(brace[1]) {
			fprintf(thread_stderr, "Error: bin must be at the end of the line\n");
			return -1;
		}
		*brace = 0;
	}
			       ...
			       const char *cwd, int clen, int required)
{
	...
	if(inputs) {
		...
		if(eval_pattern[0]) {
			fprintf(thread_stderr, "Error: bang rules can't have normal inputs, only order-only inputs. Pattern was: %s\n", input_pattern);
			return -1;
		}
	}
	free(eval_pattern);
			...
			const char *cwd, int clen)
{
	...
	if(r->command[0] == '*') {
		...
		if(!st) {
			fprintf(thread_stderr, "Error: Unable to find *-chain: '%s'\n", r->command);
			return -1;
		}
		ch = container_of(st, struct chain, st);
		...
		if(!r->inputs.num_entries && r->output_nl) {
			...
			if(!r->inputs.num_entries) {
				fprintf(thread_stderr, "Error: Unable to find any inputs for the *-chain for output '%s' in rule at line %i\n", last_output_pattern, r->line_number);
				return -1;
			}
		}
		...
		list_for_each_entry(bal, banglist, list) {
			if(bal->list.next == banglist) {
				...
				if(strcmp(bal->br->output_pattern, "") == 0) {
					fprintf(thread_stderr, "Error: Intermediate !-macro '%s' in *-chain '%s' does not specify an output pattern.\n", bal->br->st.s, ch->st.s);
					return -1;
				}
			}
				...
				struct bin_list *bl, const char *cwd, int clen)
{
	...
	if(!r->foreach) {
		fprintf(thread_stderr, "Error: reverse rule must use 'foreach'\n");
		return -1;
	}
	if(!r->input_pattern) {
		fprintf(thread_stderr, "Error: reverse rule must have input list\n");
		return -1;
	}
			       ...
			       int lno, int required)
{
	...
	tupid_tree_for_each(symid, rbn, &symtree) {
		tent = tup_entry_get(symid);
		if(tent->type == TUP_NODE_GENERATED) {
			fprintf(thread_stderr, "Error: Attempted to input files using a symlink that was generated by tup. Directory %lli, symlink used was %lli, line number %i\n", tf->tupid, tent->tnode.tupid, lno);
			sym_bork = 1;
		}
	}
	if(sym_bork)
			 ...
			 struct bin_list *bl, struct rb_root *symtree)
{
	...
	do {
		...
		if(p[0] == '{') {
			...
			if(!bl) {
				fprintf(thread_stderr, "Parse error: Bins are only usable in an input or output list.\n");
				return -1;
			}

			endb = strchr(p, '}');
			if(!endb) {
				fprintf(thread_stderr, "Parse error: Expecting end bracket for input bin.\n");
				return -1;
			}
			*endb = 0;
			pl->bin = bin_find(p+1, bl);
			if(!pl->bin) {
				fprintf(thread_stderr, "Parse error: Unable to find bin '%s'\n", p+1);
				return -1;
			}
		} else {
			struct pel_group pg;
			...
			if(pg.pg_flags & PG_HIDDEN) {
				fprintf(thread_stderr, "Error: You specified a path '%s' that contains a hidden filename (since it begins with a '.' character). Tup ignores these files - please remove references to it from the Tupfile.\n", p);
				return -1;
			}
			pl->dt = find_dir_tupid_dt_pg(dt, &pg, &pl->pel, NULL, symtree, 0);
			if(pl->dt <= 0) {
				fprintf(thread_stderr, "Error: Failed to find directory ID for dir '%s' relative to %lli\n", p, dt);
				return -1;
			}
		       ...
		       struct name_list *nl, int required)
{
	...
	if(char_find(pl->pel->path, pl->pel->len, "*?[") == 0) {
		...
		if(!tent) {
			if(!required)
				return 0;
			fprintf(thread_stderr, "Error: Explicitly named file '%.*s' not found in subdir %lli.\n", pl->pel->len, pl->pel->path, pl->dt);
			tup_db_print(thread_stderr, pl->dt);
			return -1;
		}
		if(tent->type == TUP_NODE_GHOST) {
			if(!required)
				return 0;
			fprintf(thread_stderr, "Error: Explicitly named file '%.*s' is a ghost file, so it can't be used as an input.\n", pl->pel->len, pl->pel->path);
			tup_db_print(thread_stderr, tent->tnode.tupid);
			return -1;
		}
		if(tupid_tree_search(&tf->g->delete_tree, tent->tnode.tupid) != NULL) {
			if(!required)
				return 0;
			fprintf(thread_stderr, "Error: Explicitly named file '%.*s' in subdir %lli is scheduled to be deleted (possibly the command that created it has been removed).\n", pl->pel->len, pl->pel->path, pl->dt);
			tup_db_print(thread_stderr, pl->dt);
			return -1;
		}
		if(build_name_list_cb(&args, tent) < 0)
		   ...
		   struct name_list *output_nl)
{
	...
	while(!list_empty(&oplist)) {
		...
		if(pl->path) {
			fprintf(thread_stderr, "Error: Attempted to create an output file '%s', which contains a '/' character. Tupfiles should only output files in their own directories.\n - Directory: %lli\n - Rule at line %i: [35m%s[0m\n", pl->path, tf->tupid, r->line_number, r->command);
			return -1;
		}
		...
		if(name_cmp(onle->path, "Tupfile") == 0 ||
		   ...
		   name_cmp(onle->path, "tup.config") == 0) {
			fprintf(thread_stderr, "Error: Attempted to generate a file called '%s', which is reserved by tup. Your build configuration must be comprised of files you write yourself.\n", onle->path);
			free(onle->path);
			free(onle);
			return -1;
		}
...
out_pl:
	...
	if(tupid_tree_insert(&tf->cmd_tree, cmd_tt) < 0) {
		fprintf(thread_stderr, "Error: Attempted to add duplicate command ID %lli\n", cmdid);
		tup_db_print(thread_stderr, cmdid);
		return -1;
	}
	...
	while(!list_empty(&onl.entries)) {
		...
		if(tup_db_create_unique_link(cmdid, onle->tent->tnode.tupid, &tf->g->delete_tree, &tree) < 0) {
			fprintf(thread_stderr, "You may have multiple commands trying to create file '%s'\n", onle->path);
			return -1;
		}
			...
			int is_command)
{
	...
	if(!nl) {
		fprintf(thread_stderr, "tup internal error: tup_printf called with NULL name_list\n");
		return NULL;
	}
	...
	while((next = find_char(p, cmd+cmd_len - p, '%')) !=  NULL) {
		...
		if(next == cmd+cmd_len-1) {
			fprintf(thread_stderr, "Error: Unfinished %%-flag at the end of the string '%s'\n", cmd);
			return NULL;
		}
		next++;
		p = next+1;
		...
		if(*next == 'f') {
			if(nl->num_entries == 0) {
				fprintf(thread_stderr, "Error: %%f used in rule pattern and no input files were specified.\n");
				return NULL;
			}
			clen += nl->totlen + space_chars;
		} else if(*next == 'b') {
			if(nl->num_entries == 0) {
				fprintf(thread_stderr, "Error: %%b used in rule pattern and no input files were specified.\n");
				return NULL;
			}
			...
			if(!ext) {
				fprintf(thread_stderr, "Error: %%e is only valid with a foreach rule for files that have extensions.\n");
				if(nl->num_entries == 1) {
					nle = list_entry(nl->entries.next,
							 struct name_list_entry,
							 list);
					fprintf(thread_stderr, " -- Path: '%s'\n", nle->path);
				} else {
					fprintf(thread_stderr, " -- This does not appear to be a foreach rule\n");
				}
				return NULL;
			}
			clen += extlen;
			...
			if(!is_command) {
				fprintf(thread_stderr, "Error: %%o can only be used in a command.\n");

				return NULL;
			}
			if(onl->num_entries == 0) {
				fprintf(thread_stderr, "Error: %%o used in rule pattern and no output files were specified.\n");
				return NULL;
			}
			clen += onl->totlen + (onl->num_entries-1);
		} else if(*next == 'O') {
			if(!onl) {
				fprintf(thread_stderr, "Error: %%O can only be used in the extra outputs section.\n");
				return NULL;
			}
			if(onl->num_entries != 1) {
				fprintf(thread_stderr, "Error: %%O can only be used if there is exactly one output specified.\n");
				return NULL;
			}
	...
	while((next = find_char(p, cmd+cmd_len - p, '%')) !=  NULL) {
		...
		if(*next == 'f') {
			...
			fprintf(thread_stderr, "tup internal error: Unhandled %%-flag '%c'\n", *next);
			return NULL;
		}
	}
	memcpy(&s[x], p, cmd+cmd_len - p + 1);
	if((signed)strlen(s) != clen) {
		fprintf(thread_stderr, "Error: Calculated string length (%i) didn't match actual (%li). String is: '%s'.\n", clen, (long)strlen(s), s);
		return NULL;
	}
	return s;
}
		  ...
		  const char *cwd, int clen)
{
			...
			if(s[1] == '(') {
				...
				if(rparen-var == 7 &&
				   strncmp(var, "TUP_CWD", 7) == 0) {
					if(!cwd) {
						fprintf(thread_stderr, "Error: TUP_CWD is only valid in variable and rule definitions.\n");
						return NULL;
					}
					len += clen;
		...
		if(*s == '\\') {
			...
			if(s[1] == '(') {
				...
				if(rparen-var == 7 &&
				   strncmp(var, "TUP_CWD", 7) == 0) {
					if(!cwd) {
						fprintf(thread_stderr, "Error: TUP_CWD is only valid in variable definitions.\n");
						return NULL;
					}
					memcpy(p, cwd, clen);
					p += clen;
	...
	if((signed)strlen(ret) != len) {
		fprintf(thread_stderr, "Length mismatch: expected %i bytes, wrote %li\n", len, (long)strlen(ret));
		return NULL;
	}
...
syntax_error:
	fprintf(thread_stderr, "Syntax error: expected %s\n", expected);
	return NULL;
}
