...
{
	int dbrc;
	sqlite3_stmt **stmt = &stmts[DB_SELECT_FLAGS];
				...
				void *arg, int flags)
{
	int rc;
	int dbrc;
	sqlite3_stmt **stmt = &stmts[DB_SELECT_NODE_BY_FLAGS];
	static char s[] = "select id, dir, name, type, flags from node where flags&?";
...
{
	int rc;
	sqlite3_stmt **stmt = &stmts[DB_SET_FLAGS_BY_ID];
