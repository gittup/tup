...
enum {
	...
	_DB_VARIANT_DIR_RECLAIMABLE1,
	_DB_VARIANT_DIR_RECLAIMABLE2,
	_DB_GET_DB_VAR_TREE,
	_DB_VAR_FLAG_DIRS,
...
/* If any non-ghosts exist in the variant dir, we can't be reclaimed. */
static int variant_dir_reclaimable1(tupid_t tupid)
{
	int rc = -1;
	int dbrc;
	sqlite3_stmt **stmt = &stmts[_DB_VARIANT_DIR_RECLAIMABLE1];
	static char s[] = "select exists(select 1 from node where dir=? and type!=?)";

	transaction_check("%s [37m[%lli, %i][0m", s, tupid, TUP_NODE_GHOST);
	if(!*stmt) {
		if(sqlite3_prepare_v2(tup_db, s, sizeof(s), stmt, NULL) != 0) {
			fprintf(stderr, "SQL Error: %s\n", sqlite3_errmsg(tup_db));
			fprintf(stderr, "Statement was: %s\n", s);
	...
	if(rc == SQLITE_DONE) {
		fprintf(stderr, "tup error: Expected variant_dir_reclaimable1() to get an SQLite row returned.\n");
		goto out_reset;
	}
	if(rc != SQLITE_ROW) {
		fprintf(stderr, "SQL step error: %s\n", sqlite3_errmsg(tup_db));
	...
	if(dbrc == 0) {
		...
		fprintf(stderr, "tup error: Expected variant_dir_reclaimable1() to get a 0 or 1 from SQLite\n");
		goto out_reset;
	}
...
/* If a valid file still points to our directory, then we can't be reclaimed
 * yet. For example, if the original Tupfile still exists but we don't have any
 * generated files in this variant, we still want to hang around so that we can
 * be parsed later.
 */
static int variant_dir_reclaimable2(tupid_t tupid)
{
	int rc = -1;
	int dbrc;
	sqlite3_stmt **stmt = &stmts[_DB_VARIANT_DIR_RECLAIMABLE2];
	static char s[] = "select exists(select 1 from link, node where to_id=? and id=from_id and type!=?)";
	...
	if(rc == SQLITE_DONE) {
		fprintf(stderr, "tup error: Expected variant_dir_reclaimable2() to get an SQLite row returned.\n");
		goto out_reset;
	}
	if(rc != SQLITE_ROW) {
		fprintf(stderr, "SQL step error: %s\n", sqlite3_errmsg(tup_db));
	...
	if(dbrc == 0) {
		...
		fprintf(stderr, "tup error: Expected variant_dir_reclaimable2() to get a 0 or 1 from SQLite\n");
		goto out_reset;
	}
...
int tup_db_variant_dir_reclaimable(tupid_t dt, int *reclaimable)
{
	...
	*reclaimable = 0;
	...
	rc1 = variant_dir_reclaimable1(dt);
	rc2 = variant_dir_reclaimable2(dt);
